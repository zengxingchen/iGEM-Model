...
from TissueSystem import TissueSystem
cellSystem = CellSystem()
tissueSystem = TissueSystem()

def setup(): 
    size(1700,1400)  #Modifying here you also need to change the class variable size in the Cell class
    frameRate(40)
...
def draw():
    ...
    tissueSystem.display()
    tissueSystem.update()
    cellSystem.display()
    cellSystem.update()
    cellSystem.check()
    tissueSystem.insulin_check(cellSystem.checkinsulin())
    tissueSystem.check()
    
    if frameCount % 40 == 0:  #Call one every 40 framesy
        tissueSystem.add_glu_out()
    
    if frameCount % 40 == 0 and cellSystem.protein_system.count_protein_num(type = "Complex", status = 0) < 10: #Call one every 240 frames
        cellSystem.protein_system.add(cellSystem.sensor_system.sensor_list[1].position, type = 'Complex', status = 0)
    '''
    drawText()
...
def drawText():
    textFont(f,14)
    fill(0,255,255)
    text("GIP",3*width/8 - 10, height/3)
