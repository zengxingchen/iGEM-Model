...
def mousePressed():
    if ((PVector.dist(PVector(mouseX, mouseY), cellSystem.cell.position) <= cellSystem.cell.r) and 
        (PVector.dist(PVector(mouseX, mouseY), cellSystem.sensor_system.sensor_list[1].position) >= cellSystem.sensor_system.sensor_list[1].r)):
...
def drawText():
    ...
    #text:
    fill(255,165,0)
    text("insulin_in", width - 200, height/2 - 420)
    text("insulin_out", width - 200, height/2 - 400)
    text("mRNA", width - 200, height/2 - 380)
    text("miRNA",width - 200, height/2 - 360)
    text("RAS",width - 200, height/2 - 340)
    # text("Gal4",width - 200, height/2 - 320)
    # text("Complex",width - 200, height/2 - 300)
    
    #num: give a different color to display 
    fill(255,0,255)
    text(cellSystem.count_protein_num(type = "insulin", status = 0), width - 100, height/2 - 420)
    text(cellSystem.count_protein_num(type = "insulin", status = 2), width - 100, height/2 - 400)
    text(cellSystem.protein_system.count_mrna_num(), width - 100, height/2 - 380)
    text(cellSystem.protein_system.count_mirna_num(),width - 100, height/2 - 360)
    text(cellSystem.count_protein_num(type = "RAS", status = 0),width - 100, height/2 - 340)
